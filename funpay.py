...
def raiseOrder():
    driver.find_element_by_xpath("//button[@class='btn btn-default btn-block js-lot-raise']").click()
...
def main():
    ...
    try:  
        ...
        driver.refresh()
        print('[Prog] Вошёл в аккаунт')
        #delay = int(input('[Prog] Введите вашу задержку (в минутах)')) # отголоски интерфейса
        ...
        while True:
            i = 1
            print('[Prog] Начинаю проход по лотам')
            while True:
                i += 1
                try:
            ...
            print(f'Жду {delay//60} минут (Всего работаю: {timer_string})')
            time.sleep(delay)
            timer_min += delay / 60
            if timer_min % 60 == 0:
                timer_min = 0
